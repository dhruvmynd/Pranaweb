/*
  # Add HRV Tracking to User Analytics
  
  1. Changes
    - Add avg_hrv column to user_analytics table
    - Update calculate_user_analytics function to include HRV calculation
    - Modify update_user_analytics trigger function to handle HRV data
    
  2. Benefits
    - Track average HRV across all sessions
    - Provide consolidated HRV metrics for users
    - Enable HRV trend analysis in the dashboard
*/

-- Add avg_hrv column to user_analytics
ALTER TABLE user_analytics
ADD COLUMN avg_hrv numeric(10,2) DEFAULT 0;

-- Add sessions_with_hrv column to track how many sessions have HRV data
ALTER TABLE user_analytics
ADD COLUMN sessions_with_hrv integer DEFAULT 0;

-- Update the calculate_user_analytics function to include HRV
...
BEGIN
  ...
  SELECT 
    COALESCE(AVG(hrv), 0) as avg_hrv,
    COUNT(CASE WHEN hrv > 0 THEN 1 END) as hrv_count
  INTO practice_hrv, practice_hrv_count
  FROM practice_sessions
  WHERE user_id = user_id_param AND hrv > 0;
...
CREATE INDEX IF NOT EXISTS idx_user_analytics_avg_hrv 
ON user_analytics(avg_hrv);

-- Update all existing user analytics to include HRV data
UPDATE user_analytics
SET updated_at = now();

-- Add comment explaining the changes
COMMENT ON COLUMN user_analytics.avg_hrv IS 
'Average Heart Rate Variability across all sessions in milliseconds';

COMMENT ON COLUMN user_analytics.sessions_with_hrv IS 
'Number of sessions with valid HRV measurements';